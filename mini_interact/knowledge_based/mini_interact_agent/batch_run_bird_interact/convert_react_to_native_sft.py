...
STEP_RE = re.compile(
    ...
    r'Observation:\s*(?P<obs>.*?)(?=<thought>|\Z)',
    re.DOTALL,
)
...
def render_history_text(steps, start_turn=1):
    ...
    for i, (thought, action_str, obs) in enumerate(steps):
        a_content, _ = make_assistant_content(thought, action_str)
        tr = make_tool_response(obs, action_str, start_turn + i)
...
def convert_standard(sample):
    ...
    while i < len(msgs):
        ...
        if m["role"] == "assistant":
            ...
            content, _ = make_assistant_content(think, am.group(1).strip())
            out.append({"role": "assistant", "content": content})
            # following user/observation (if any) becomes a tool_response
            if i + 1 < len(msgs) and msgs[i + 1]["role"] == "user":
                tr = make_tool_response(msgs[i + 1]["content"], am.group(1).strip(), turn)
...
def convert_recovery(sample):
    ...
    corr_content, _ = make_assistant_content(tm.group(1) if tm else "", am.group(1).strip())
