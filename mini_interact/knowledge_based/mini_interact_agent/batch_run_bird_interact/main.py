...
from batch_run_bird_interact import action_handler as pg_handler
...
def run_batch_evaluation(args):
    ...
    if args.debug:
        logger.warning("DEBUG MODE: Will directly submit solution SQLs to test handlers")
    if args.resume:
    ...
    # Select action handler backend and bind references (no mid-function imports)
    if args.mini_interact:
        handler = sqlite_handler
        def _resolve_db_identifier(db_name: str, record: Dict[str, Any]):
            explicit = record.get('db_file_path') if isinstance(record, dict) else None
            return explicit if explicit else os.path.join(data_path_base, db_name, f"{db_name}.sqlite")
    else:
        handler = pg_handler
        def _resolve_db_identifier(db_name: str, record: Dict[str, Any]):
            return db_name

    execute_env_action = handler.execute_env_action
    execute_submit_action = handler.execute_submit_action
    _schema_cache = handler._schema_cache
    load_db_data_if_needed = handler.load_db_data_if_needed
    close_db_connection = handler.close_db_connection
    ...
    # Debug mode: Directly submit solution SQLs to test handlers
    if args.debug:
        logger.warning("=== DEBUG MODE: Directly submitting solution SQLs ===")
        for status in all_statuses:
            record = status.original_data
            
            # Phase 1: Submit sol_sql
            sol_sql_phase1 = record.get('sol_sql', '')
            if isinstance(sol_sql_phase1, list):
                sol_sql_phase1 = '\n'.join(sol_sql_phase1) if sol_sql_phase1 else ''
            elif not isinstance(sol_sql_phase1, str):
                sol_sql_phase1 = str(sol_sql_phase1) if sol_sql_phase1 else ''
            
            if sol_sql_phase1:
                # Test execute() action first
                logger.warning(f"Sample {status.idx}: Testing execute() with Phase 1 solution SQL...")
                execute_action = f"execute({sol_sql_phase1})"
                exec_observation, exec_success = execute_env_action(execute_action, status, data_path_base)
                if exec_success:
                    logger.warning(f"Sample {status.idx}: execute() PASSED - {exec_observation[:100]}...")
                else:
                    logger.warning(f"Sample {status.idx}: execute() FAILED - {exec_observation}")
                
                # Now test submit() action
                logger.warning(f"Sample {status.idx}: Submitting Phase 1 solution SQL...")
                observation, reward, p1_comp, p2_comp, finished = execute_submit_action(sol_sql_phase1, status, data_path_base)
                status.phase1_completed = p1_comp
                status.phase2_completed = p2_comp
                status.last_reward = reward if reward is not None else 0.0
                ...
                if p1_comp:
                    ...
                    logger.warning(f"Sample {status.idx}: Phase 1 PASSED (Reward: {reward})")
                    
                    # Phase 2: Submit follow_up sol_sql if exists
                    if 'follow_up' in record and record['follow_up']:
                        follow_up = record['follow_up']
                        sol_sql_phase2 = follow_up.get('sol_sql', '')
                        if isinstance(sol_sql_phase2, list):
                            sol_sql_phase2 = '\n'.join(sol_sql_phase2) if sol_sql_phase2 else ''
                        elif not isinstance(sol_sql_phase2, str):
                            sol_sql_phase2 = str(sol_sql_phase2) if sol_sql_phase2 else ''
                        
                        if sol_sql_phase2:
                            logger.warning(f"Sample {status.idx}: Submitting Phase 2 solution SQL...")
                            status.current_phase = 2
                            observation, reward, p1_comp, p2_comp, finished = execute_submit_action(sol_sql_phase2, status, data_path_base)
                            status.phase1_completed = p1_comp
                            status.phase2_completed = p2_comp
                            status.last_reward = reward if reward is not None else 0.0
                            status.last_observation = observation if isinstance(observation, str) else str(observation)
                            status.current_turn = 2
                            
                            if p2_comp:
                                logger.warning(f"Sample {status.idx}: Phase 2 PASSED (Reward: {reward})")
                                status.task_finished = True
                            else:
                                logger.warning(f"Sample {status.idx}: Phase 2 FAILED: {observation}")
                                status.task_finished = True  # Mark as finished even if failed
                        else:
                            logger.warning(f"Sample {status.idx}: No Phase 2 solution SQL found")
                            status.task_finished = True
                    else:
                        logger.warning(f"Sample {status.idx}: No follow_up found, task finished after Phase 1")
                        status.task_finished = True
                else:
                    logger.warning(f"Sample {status.idx}: Phase 1 FAILED: {observation}")
                    status.task_finished = True  # Mark as finished even if failed
            else:
                logger.warning(f"Sample {status.idx}: No Phase 1 solution SQL found")
                status.task_finished = True
        ...
        logger.warning("=== DEBUG MODE: Calculating metrics ===")
        ...
        logger.warning(f"Debug metrics saved to: {metrics_path}")
        ...
        for db_name in connected_dbs:
            record = next((s.original_data for s in all_statuses if s.original_data['selected_database'] == db_name), {})
            identifier = _resolve_db_identifier(db_name, record)
            close_db_connection(identifier)
        
        logger.warning("=== DEBUG MODE: Complete ===")
        return
            ...
            if valid_ask_statuses:
                if args.user_sim_mode == 'vanilla':
                    ...
                    for i, status in enumerate(valid_ask_statuses):
                        ...
                        encoder_prompt = build_user_encoder_prompt(question, status, db_schema, args.user_sim_prompt_version)
                    ...
                    for i, status in enumerate(valid_ask_statuses):
                        if status.idx in encoder_results:
                            ...
                            load_db_data_if_needed(db_name, data_path_base) # Ensure schema is loaded
                            db_schema = _schema_cache.get(db_name, "Schema not available")
                            decoder_prompt = build_user_decoder_prompt(question, encoder_action, status, db_schema, args.user_sim_prompt_version)
...
def main():
    ...
    parser.add_argument('--mini_interact', action='store_true', help='Use mini_interact (SQLite) backend instead of PostgreSQL')
    ...
    # Debug Configuration
    parser.add_argument('--debug', action='store_true', help='Debug mode: directly submit solution SQLs to test handlers (skips agent interaction)')
    ...
    # Set database configuration (PostgreSQL only)
    if not args.mini_interact:
        set_global_db_config(host=args.db_host, port=args.db_port)
    try:
        run_batch_evaluation(args)
    finally:
        # Ensure DB config is reset even if errors occur (PostgreSQL only)
        if not args.mini_interact:
